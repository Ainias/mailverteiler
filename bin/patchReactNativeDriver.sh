...
FILE=node_modules/typeorm/browser/driver/react-native/ReactNativeDriver.js
if test -f "$FILE"; then
  replace 'const sqlite = this.options.driver || require("react-native-sqlite-storage")' 'const sqlite = this.options.driver' $FILE
fi
